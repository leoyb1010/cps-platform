...
export class SettlementService {
  ...
  async applyRefundReversal(
    ...
    let share = rawShare
    const hit = await tx.settlement.updateMany({
      where: { id: s.id, agentPayout: { gte: rawShare } },
      data: { reversal: { increment: rawShare }, agentPayout: { decrement: rawShare } },
    })
    if (hit.count === 0) {
      // 余额不足：读回当前 agentPayout，按实际可扣额扣（下限 0，不透支、不产生负 agentPayout）
  ...
  async applyAgentRefundImpact(
    ...
    let deducted = 0
    const hit = await tx.agent.updateMany({ where: { id: a.id, payoutPending: { gte: share } }, data: { payoutPending: { decrement: share } } })
    if (hit.count > 0) {
      deducted = share
    } else {
