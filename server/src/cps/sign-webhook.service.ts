...
export class SignWebhookService {
  ...
  private async attempt(id: string, callbackUrl: string, payload: Record<string, unknown>): Promise<void> {
    ...
    // P1-S1：真正 fetch 前对品牌可控 callbackUrl 做 SSRF 校验，非法则标记该投递死信（不抛断主流程）。
    try {
      assertSafeCallbackUrl(callbackUrl)
    } catch (e) {
      await this.update(id, { httpStatus: 0, ok: false, error: e instanceof Error ? e.message : '回调地址不安全', deliveryStatus: 'dead', attempts: { increment: 1 }, nextRetryAt: null })
      this.logger.error(`[SSRF拦截] 回调地址非法，标记死信 ${id}：${callbackUrl}`)
      return
    }
    try {
