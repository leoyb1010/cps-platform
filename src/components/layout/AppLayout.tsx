...
import { Search, Bell, ChevronUp, Menu, X, LogOut, UserCog, Repeat, HelpCircle, Sun, Moon, MonitorSmartphone, ChevronDown, Pin, Star, Rows3 } from 'lucide-react'
import { NAV, NAV_BY_TO } from './nav'
import { GuideDrawer } from './GuideDrawer'
import { OfflineBanner } from './OfflineBanner'
import { PageSkeleton } from './PageSkeleton'
import { GUIDES } from '../../lib/guides'
import { useViewMode, setViewMode, useTheme, setTheme, type Theme, usePinned, togglePinned, useDensity, setDensity } from '../../lib/prefs'
...
function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // 单组展开：默认只露出当前任务相关入口，避免十几个模块同时抢注意力。
  const inGroup = (g: (typeof groups)[number]) => g.items.some((it) => it.to === loc.pathname || (it.activeMatch?.includes(loc.pathname) ?? false))
  const activeGroupTitle = groups.find(inGroup)?.title
  const [expandedGroup, setExpandedGroup] = useState<string | null>(() => activeGroupTitle ?? null)
  useEffect(() => { if (activeGroupTitle) setExpandedGroup(activeGroupTitle) }, [activeGroupTitle])
  ...
  return (
    <>
      ...
      >
        ...
        <nav data-coach="nav" className="flex-1 overflow-y-auto px-3 pb-5 pt-2">
          ...
          {groups.map((group, gi) => {
            const forced = gi === 0 || inGroup(group)
            const isCollapsed = gi > 0 && !forced && expandedGroup !== group.title
            const badge = groupBadge(group)
            const collapsible = gi > 0
            return (
              <div key={group.title} className="mb-1">
                {gi > 0 && (
                  <button
                    onClick={() => setExpandedGroup((current) => current === group.title && !forced ? null : group.title)}
                    className={cx(
                      'flex w-full items-center gap-2 rounded-md px-2 py-2 text-[11px] font-semibold tracking-[0.06em] text-ink-4 transition-colors',
                      collapsible && 'cursor-pointer hover:bg-surface-muted hover:text-ink-2',
                    )}
...
function Topbar({ title, base, onMenu, onOpenGuide }: { title: string; base: string; onMenu: () => void; onOpenGuide: () => void }) {
          ...
          <div className="absolute right-2.5 bottom-[58px] left-2.5 z-[56] rounded-xl border border-line bg-surface py-1.5 shadow-[var(--shadow-pop)]" style={{ animation: 'revUpSm .18s both' }}>
            {!roles ? (
              <>
                ...
                {(user.roleId === 'super' || user.roleId === 'audit') && (
                  <div className="flex items-center justify-between px-3 py-1.5">
                    <span className="flex items-center gap-2.5 text-[12.5px] text-ink-2"><Rows3 size={14} className="text-ink-4" /> 页面信息</span>
                    <Segmented value={mode} onChange={setViewMode} options={[{ value: 'simple', label: '核心' }, { value: 'expert', label: '完整' }]} />
                  </div>
                )}
  ...
  return (
    <ReplayContext.Provider value={{ epoch, replay }}>
      <div className="min-h-screen">
        ...
        <div className="flex min-h-screen flex-col bg-canvas md:pl-[236px]">
          <Topbar title={title} base={base} onMenu={() => setNavOpen(true)} onOpenGuide={() => setGuideOpen(true)} />
        ...
        {base === '/' && user?.roleId === 'super' && (
          <CoachMarks
            ...
            steps={[
              ...
              { anchor: '[data-coach="nav"]', title: '按工作找入口', body: '导航按业务增长、资金与风险、合作工具分类；一次只展开当前一组，完整能力仍可随时进入。' },
            ]}
          />
        )}
      </div>
