      ...
      <Modal open onClose={onClose} title="成员已创建" footer={<Button variant="primary" onClick={onClose}>完成</Button>}>
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between rounded-lg border border-line bg-surface-muted p-3">
            ...
            <button onClick={() => { void copyText(created.tempPassword).then((ok) => toast(ok ? { tone: 'good', text: '已复制' } : { tone: 'info', text: '复制失败，请手动复制' })) }} className="tnum rounded-md bg-surface px-3 py-1.5 text-[13px] font-semibold text-ink ring-1 ring-line hover:ring-brand/40">{created.tempPassword}</button>
          </div>
