...
export default function Audit() {
  ...
  const rows = s.activity.map((a) => ({ ...a, cat: CAT_OF(a.text) }))
  ...
  const exportCsv = () => {
    ...
    const body = list.map((r) => [r.t, user?.name ?? '系统', CAT_LABEL[r.cat].label, r.text.replace(/,/g, '，')])
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Card mark><Stat label="日志条数" value={String(rows.length)} sub={<span>本会话累计</span>} /></Card>
              ...
              <Row key={r.id}>
                ...
                <Td>
                  <div className="flex items-center gap-2">
                    <span className="grid h-6 w-6 place-items-center rounded-full bg-brand-soft text-[10px] font-semibold text-brand-ink ring-1 ring-brand/15">{(user?.name ?? '系').slice(0, 1)}</span>
                    <span className="text-[12px] text-ink-2">{user?.name ?? '系统'}</span>
                  </div>
                </Td>
