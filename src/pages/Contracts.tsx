...
import { money, pct, csvCell, downloadText } from '../lib/format'

export default function Contracts() {
  const toast = useToast()
  const { brands, contracts: storeContracts } = useStore()
  // 单源：增长合约从真实后端读（Prisma）；演示模式无后端 → 回退本地 seed，页面不至于一片空白
  const contractsApi = useApi(() => bizApi.contracts<GrowthContract[]>(), [])
  const contracts = contractsApi.data ?? (isRealApi ? [] : storeContracts)
  ...
  return (
    <>
      ...
      {!isRealApi && <div className="mb-4 rounded-lg border border-dashed border-line bg-surface-muted px-3.5 py-2 text-[11.5px] text-ink-3">演示数据 · 连接真实后端后展示服务端合约</div>}
...
function ContractWizard({ brands, onClose, onDone }: { brands: { id: string; name: string }[]; onClose: () => void; onDone: (name: string) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // 演示模式（无后端）/ 网络异常：不再无声挂起，向导保持打开可重试
      toast({ tone: 'alert', text: '提交失败：' + (e instanceof Error ? e.message : '网络异常，请重试') })
